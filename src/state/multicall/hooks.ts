...
type MethodArg = string | number | boolean | BigNumber
type MethodArgs = Array<MethodArg | MethodArg[]>

export type OptionalMethodInputs = Array<MethodArg | MethodArg[] | undefined> | undefined

function isMethodArg(x: unknown): x is MethodArg {
    return BigNumber.isBigNumber(x) || ['string', 'number', 'boolean'].indexOf(typeof x) !== -1
}
...
export function useSingleCallResult(
    ...
    const calls = useMemo<Call[]>(() => {
        return contract !== undefined && fragment !== undefined && isValidMethodArgs(inputs)
            ? [
                {
                    address: contract?.address ?? '',
                    callData: contract?.interface.encodeFunctionData(fragment, inputs) ?? '',
