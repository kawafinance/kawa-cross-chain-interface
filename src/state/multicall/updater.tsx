...
async function fetchChunk (
  ...
  try {
    ...
    console.debug('Fetched chunk', blockNumber, returnData);
...
export default function Updater (): null {
  ...
  const multicall3Contract = useMulticallContract()
  ...
  useEffect(() => {
    if (!latestBlockNumber || !chainId || !multicall3Contract) return
    ...
    cancellations.current = {
      ...
      cancellations: chunkedCalls.map((chunk, index) => {
        const { cancel, promise } = retry(() => fetchChunk(multicall3Contract, chunk, latestBlockNumber), {
          n: Infinity,
          minWait: 1000,
          maxWait: 3500
        })
  ...
  }, [chainId, multicall3Contract, dispatch, serializedOutdatedCallKeys, latestBlockNumber])
