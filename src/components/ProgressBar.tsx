...
const ProgressBar = ({borrowBalance, max, className = ''}) => {
    ...
    const percentage = useMemo(() =>
            borrowBalance > 0 && max > 0
                ? Math.min(borrowBalance / max * 100, 100)
                : 0
        , [borrowBalance, max])
    ...
    const borrowBalanceFormatted = formatNumber(borrowBalance, true)
...
//The borrowing capacity bar displays the total amount you can borrow ($1.52). Your liquidation threshold will be hit if the bar hits 100% ($1.59), which will result in some or all of your collateral becoming eligible to be liquidated.
//